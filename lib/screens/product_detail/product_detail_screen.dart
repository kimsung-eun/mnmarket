...
class ProductDetailScreen extends StatefulWidget {
  ...
  @override State<ProductDetailScreen> createState() => _D();
}
...
class _D extends State<ProductDetailScreen> {
  ...
  String formatNumber(int x) =>
    x.toString().replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+$)'), (m) => '${m[1]},');
          ...
          child: ElevatedButton(
            onPressed: () => showDialog(
              ...
              builder: (_) => AlertDialog(
                ...
                content: Text('${p.name} $qty개 구매 완료 (${formatNumber(qty * p.price)}원)'),
